...
export class PossessionPage {
  ...
  async gotoPossessionPage(): Promise<void> {
    await this.page.goto('/possession');
  }

  possessionRow(name: string): Locator {
    return this.page.getByRole('row').filter({
      has: this.page.getByRole('cell', { name, exact: true }),
    });
  }

  async addPossession(name: string, isDefault = false): Promise<void> {
  ...
  async searchPossession(name: string): Promise<void> {
    await this.searchInput.fill(name);
    await this.searchInput.press('Enter');
  }

  async viewPossession(name: string): Promise<void> {
    await this.possessionRow(name).getByTitle('View', { exact: true }).click();
  }

  async editPossession(name: string, updatedName: string, isDefault = false): Promise<void> {
    await this.possessionRow(name).getByTitle('Edit', { exact: true }).click();
  ...
  async deactivatePossession(name: string): Promise<void> {
    await this.possessionRow(name).getByTitle('Deactivate', { exact: true }).click();
    await this.page.getByRole('button', { name: 'Yes', exact: true }).click();
  }

  async restorePossession(name: string): Promise<void> {
    await this.possessionRow(name).getByTitle('Restore', { exact: true }).click();
    await this.page.getByRole('button', { name: 'Yes', exact: true }).click();
  }
